class rijndael{
    ...
    private static Te0:Array<number>=[
    ...
    private static Te1:Array<number>=[
    ...
    private static Te2:Array<number>=[
    ...
    private static Te3:Array<number>=[
    ...
    private static Te4:Array<number>=[
    ...
    private static Td0:Array<number>=[
    ...
    private static Td1:Array<number>=[
    ...
    private static Td2:Array<number>=[
    ...
    private static Td3:Array<number>=[
    ...
    private static Td4:Array<number>=[
    ...
    private static rco:Array<number>=[
        0x01,0x02,0x04,0x08,0x10,
        0x20,0x40,0x80,0x1B,0x36
    ];

    //Key schedule arrays
    private Nr:number = 0;//Number of rounds [For 128 bit block, Nr = {10, 12, 14} for 128, 192, 256 bit cipher key]

    private static fkey:Array<number>;// [60];//Nb*(Nr + 1)
    private static rkey:Array<number>;// [60];//Nb*(Nr + 1)

    public static KEYLENGTH:number = 16;
    public static BLOCKSIZE:number = 16;


    public constructor() {
        rijndael.fkey = new Array<number>(60);
        rijndael.rkey = new Array<number>(60);
    }
    ...
    private CreateDecryptionKeys(Nb:number = 0):void{
        var i:number=0;
        var j:number = Nb * this.Nr;
        var arr:Array<number> = new Array<number>(4);
        var s:egret.ByteArray = new egret.ByteArray();
        s.endian = egret.Endian.LITTLE_ENDIAN;
        //Invert the order of the round keys
        for (var k:number = 0; k <= this.Nr; k++, i += Nb, j -= Nb) {
            for (var l = 0; l < Nb; l++ )
                rijndael.rkey[i + l] = rijndael.fkey[j + l];
        }
        //Apply the inverse MixColumn transform to all round keys except the first and the last
        for (i = Nb; i < Nb * this.Nr; i++){
            s.position = 0;
            //s.writeUnsignedInt(rijndael.rkey[i]);
            CPacket.writeBinInt(s,rijndael.rkey[i]);
            s.position = 0;
            arr[0]=s.readByte()&0xff;
            arr[1]=s.readByte()&0xff;
            arr[2]=s.readByte()&0xff;
            arr[3]=s.readByte()&0xff;

            rijndael.rkey[i]=rijndael.Td0[rijndael.Te4[arr[0]]&0xFF]^rijndael.Td1[rijndael.Te4[arr[1]]&0xFF]^rijndael.Td2[rijndael.Te4[arr[2]]&0xFF]^rijndael.Td3[rijndael.Te4[arr[3]]&0xFF];
        }
    }


    //Key Scheduler. Expand the cipher key into the encryption key schedule.
    //pass(0 ... n-1) contains the cipher key, where n = 32.
    public SetCipherKey(key:egret.ByteArray):void{
        //128 bit block size
        var _key:egret.ByteArray = new egret.ByteArray();
        _key.endian = egret.Endian.LITTLE_ENDIAN;
        _key.writeBytes(key, 0, Math.min(key.length, rijndael.KEYLENGTH));
        while (_key.length < rijndael.KEYLENGTH) _key.writeByte(0);
        _key.position = 0;
        rijndael.fkey[0] = _key.readUnsignedInt();
        rijndael.fkey[1] = _key.readUnsignedInt();
        rijndael.fkey[2] = _key.readUnsignedInt();
        rijndael.fkey[3] = _key.readUnsignedInt();

        var arr:Array<number> = new Array<number>(4);
        var s:egret.ByteArray = new egret.ByteArray();
        s.endian = egret.Endian.LITTLE_ENDIAN;
        for (var j:number = 0, i:number = 4; j <= 9; j++, i += 4){
            s.position = 0;
            //s.writeUnsignedInt(rijndael.fkey[i-1]);
            CPacket.writeBinInt(s,rijndael.fkey[i-1]);
            s.position = 0;
            arr[0]=s.readByte()&0xff;
            arr[1]=s.readByte()&0xff;
            arr[2]=s.readByte()&0xff;
            ...
            rijndael.fkey[i+0]=rijndael.fkey[i-4]^rijndael.Te4[arr[0]]&0xFF000000^rijndael.Te4[arr[3]]&0xFF0000^rijndael.Te4[arr[2]]&0xFF00^rijndael.Te4[arr[1]]&0xFF^rijndael.rco[j];
            rijndael.fkey[i+1]=rijndael.fkey[i-3]^rijndael.fkey[i+0];
            rijndael.fkey[i+2]=rijndael.fkey[i-2]^rijndael.fkey[i+1];
            rijndael.fkey[i+3]=rijndael.fkey[i-1]^rijndael.fkey[i+2];
        }
        this.Nr=10;
        this.CreateDecryptionKeys(4);
    }

    //Encrypt a 128 bit block.  plaintext[] is input, ciphertext[] is output.
    //plaintext and ciphertext can be the same array.
    public BlockEncrypt(plaintext:egret.ByteArray,ciphertext:egret.ByteArray):void{
        var arr:Array<number> = new Array<number>(16);
        var s:egret.ByteArray = new egret.ByteArray();
        s.endian = egret.Endian.LITTLE_ENDIAN;

        s.writeBytes(plaintext, 0, rijndael.BLOCKSIZE);
        s.position = 0;
        var p:Array<number> = new Array<number>(4);
        p[0] = s.readUnsignedInt();
        p[1] = s.readUnsignedInt();
        p[2] = s.readUnsignedInt();
        p[3] = s.readUnsignedInt();

        var t:Array<number>=[p[0]^rijndael.fkey[0],p[1]^rijndael.fkey[1],p[2]^rijndael.fkey[2],p[3]^rijndael.fkey[3]];
        for (var i:number = 1, k:number = 4; i < this.Nr; i++, k += 4) //Nr is number of rounds
        {
            s.position = 0;
//            s.writeUnsignedInt(t[0]);
//            s.writeUnsignedInt(t[1]);
//            s.writeUnsignedInt(t[2]);
//            s.writeUnsignedInt(t[3]);
            CPacket.writeBinInt(s,t[0]);
            CPacket.writeBinInt(s,t[1]);
            CPacket.writeBinInt(s,t[2]);
            CPacket.writeBinInt(s,t[3]);
            s.position = 0;
            for(var z:number=0;z<rijndael.BLOCKSIZE;z++)
                arr[z]=s.readByte()&0xff;

            t[0]=rijndael.Te0[arr[ 0]]^rijndael.Te1[arr[ 5]]^rijndael.Te2[arr[10]]^rijndael.Te3[arr[15]]^rijndael.fkey[k+0];
            t[1]=rijndael.Te0[arr[ 4]]^rijndael.Te1[arr[ 9]]^rijndael.Te2[arr[14]]^rijndael.Te3[arr[ 3]]^rijndael.fkey[k+1];
            t[2]=rijndael.Te0[arr[ 8]]^rijndael.Te1[arr[13]]^rijndael.Te2[arr[ 2]]^rijndael.Te3[arr[ 7]]^rijndael.fkey[k+2];
            t[3]=rijndael.Te0[arr[12]]^rijndael.Te1[arr[ 1]]^rijndael.Te2[arr[ 6]]^rijndael.Te3[arr[11]]^rijndael.fkey[k+3];
        }
        //Final round
        s.position = 0;
//        s.writeUnsignedInt(t[0]);
...
//        s.writeUnsignedInt(t[3]);
        ...
        t[0]=rijndael.Te4[arr[ 0]]&0xFF^rijndael.Te4[arr[ 5]]&0xFF00^rijndael.Te4[arr[10]]&0xFF0000^rijndael.Te4[arr[15]]&0xFF000000^rijndael.fkey[k+0];
        t[1]=rijndael.Te4[arr[ 4]]&0xFF^rijndael.Te4[arr[ 9]]&0xFF00^rijndael.Te4[arr[14]]&0xFF0000^rijndael.Te4[arr[ 3]]&0xFF000000^rijndael.fkey[k+1];
        t[2]=rijndael.Te4[arr[ 8]]&0xFF^rijndael.Te4[arr[13]]&0xFF00^rijndael.Te4[arr[ 2]]&0xFF0000^rijndael.Te4[arr[ 7]]&0xFF000000^rijndael.fkey[k+2];
        t[3]=rijndael.Te4[arr[12]]&0xFF^rijndael.Te4[arr[ 1]]&0xFF00^rijndael.Te4[arr[ 6]]&0xFF0000^rijndael.Te4[arr[11]]&0xFF000000^rijndael.fkey[k+3];
...
//        s.writeUnsignedInt(t[3]);
        ...
        ciphertext.writeBytes(s, 0, rijndael.BLOCKSIZE);
    }

    //Decrypt a 128 bit block.  ciphertext[] is input, plaintext[] is output.
    //plaintext and ciphertext can be the same array.
    public BlockDecrypt(ciphertext:egret.ByteArray,plaintext:egret.ByteArray):void{
        var arr:Array<number> = new Array<number>(16);
        var s:egret.ByteArray = new egret.ByteArray();
        s.endian = egret.Endian.LITTLE_ENDIAN;

        s.writeBytes(ciphertext, 0, rijndael.BLOCKSIZE);
        s.position = 0;
        var c:Array<number> = new Array<number>(4);
        c[0] = s.readUnsignedInt();
        c[1] = s.readUnsignedInt();
        c[2] = s.readUnsignedInt();
        c[3] = s.readUnsignedInt();

        var t:Array<number>=[c[0]^rijndael.rkey[0],c[1]^rijndael.rkey[1],c[2]^rijndael.rkey[2],c[3]^rijndael.rkey[3]];
        for (var i:number = 1, k:number = 4; i < this.Nr; i++, k += 4) //Nr is number of rounds
        {
            s.position = 0;
//            s.writeUnsignedInt(t[0]);
...
//            s.writeUnsignedInt(t[3]);
            ...
            t[0]=rijndael.Td0[arr[ 0]]^rijndael.Td1[arr[13]]^rijndael.Td2[arr[10]]^rijndael.Td3[arr[ 7]]^rijndael.rkey[k+0];
            t[1]=rijndael.Td0[arr[ 4]]^rijndael.Td1[arr[ 1]]^rijndael.Td2[arr[14]]^rijndael.Td3[arr[11]]^rijndael.rkey[k+1];
            t[2]=rijndael.Td0[arr[ 8]]^rijndael.Td1[arr[ 5]]^rijndael.Td2[arr[ 2]]^rijndael.Td3[arr[15]]^rijndael.rkey[k+2];
            t[3]=rijndael.Td0[arr[12]]^rijndael.Td1[arr[ 9]]^rijndael.Td2[arr[ 6]]^rijndael.Td3[arr[ 3]]^rijndael.rkey[k+3];
        }
        //Final round
        s.position = 0;
//        s.writeUnsignedInt(t[0]);
...
//        s.writeUnsignedInt(t[3]);
        ...
        t[0]=rijndael.Td4[arr[ 0]]&0xFF^rijndael.Td4[arr[13]]&0xFF00^rijndael.Td4[arr[10]]&0xFF0000^rijndael.Td4[arr[ 7]]&0xFF000000^rijndael.rkey[k+0];
        t[1]=rijndael.Td4[arr[ 4]]&0xFF^rijndael.Td4[arr[ 1]]&0xFF00^rijndael.Td4[arr[14]]&0xFF0000^rijndael.Td4[arr[11]]&0xFF000000^rijndael.rkey[k+1];
        t[2]=rijndael.Td4[arr[ 8]]&0xFF^rijndael.Td4[arr[ 5]]&0xFF00^rijndael.Td4[arr[ 2]]&0xFF0000^rijndael.Td4[arr[15]]&0xFF000000^rijndael.rkey[k+2];
        t[3]=rijndael.Td4[arr[12]]&0xFF^rijndael.Td4[arr[ 9]]&0xFF00^rijndael.Td4[arr[ 6]]&0xFF0000^rijndael.Td4[arr[ 3]]&0xFF000000^rijndael.rkey[k+3];
...
//        s.writeUnsignedInt(t[3]);
        ...
        plaintext.writeBytes(s, 0, rijndael.BLOCKSIZE);
    }

    public Encrypt(password:egret.ByteArray,plaintext:egret.ByteArray,ciphertext:egret.ByteArray):number{
        var cipherlen:number = plaintext.length + (rijndael.BLOCKSIZE-plaintext.length % rijndael.BLOCKSIZE) % rijndael.BLOCKSIZE;

        this.SetCipherKey(password);

        var plaintext_block:egret.ByteArray = new egret.ByteArray();
        var ciphertext_block:egret.ByteArray = new egret.ByteArray();

        var i:number = 0;
        for (i = 0; i + rijndael.BLOCKSIZE <= plaintext.length; i += rijndael.BLOCKSIZE){
            plaintext_block.position = 0;
            ciphertext_block.position = 0;
            plaintext_block.writeBytes(plaintext, i, rijndael.BLOCKSIZE);
            this.BlockEncrypt(plaintext_block, ciphertext_block);
            ciphertext.writeBytes(ciphertext_block, 0, rijndael.BLOCKSIZE);
        }
        if (i < plaintext.length){
            plaintext_block.clear();
            plaintext_block.position = 0;
            ciphertext_block.position = 0;
            plaintext_block.writeBytes(plaintext, i, plaintext.length % rijndael.BLOCKSIZE);
            while (plaintext_block.length < rijndael.BLOCKSIZE) plaintext_block.writeByte(0);
            this.BlockEncrypt(plaintext_block, ciphertext_block);
            ciphertext.writeBytes(ciphertext_block, 0, rijndael.BLOCKSIZE);
        }
        return cipherlen;
    ...
    public Decrypt(password:egret.ByteArray,ciphertext:egret.ByteArray,plaintext:egret.ByteArray):number{
        if (ciphertext.length % rijndael.BLOCKSIZE != 0) throw new Error();
        ...
        for (var i:number = 0; i < ciphertext.length; i += rijndael.BLOCKSIZE){
            plaintext_block.position = 0;
            ciphertext_block.position = 0;
            ciphertext_block.writeBytes(ciphertext, i, rijndael.BLOCKSIZE);
            this.BlockDecrypt(ciphertext_block, plaintext_block);
            plaintext.writeBytes(plaintext_block, 0, rijndael.BLOCKSIZE);
        }
