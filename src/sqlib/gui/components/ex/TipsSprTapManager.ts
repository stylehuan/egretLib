 ...
 * tips弹出的管理类
 */
module TipsSprTapManager {

    //tips面板
    var _tips:egret.DisplayObjectContainer;
    //tip 对象数据存贮
    // var _dict:Object;
    var _dict:Array<any> = [];
    /**
     ...
     */
    export function addTips(attachment, spr:egret.DisplayObjectContainer,effectType:number = 0):void{
        if(this._dict == null){
            this._dict = [];
        }
        ...
        {
            attachment.addEventListener(mouse.MouseEvent.MOUSE_OVER, this.showTips, this);
            attachment.addEventListener(mouse.MouseEvent.MOUSE_OUT, this.removeTips, this);
        }else
        {
        ...
        if(this._dict[attachment.hashCode] == null){
            ...
            this._dict[attachment.hashCode].spr = spr;
    ...
    export function showTips(e):void{
        ...
        var descStr:egret.DisplayObjectContainer = new egret.DisplayObjectContainer();
        var effectType:number = 0;
        if(this._tips == null){
            if(this._dict[attachment.hashCode] != null){
                descStr = this._dict[attachment.hashCode].spr;
                effectType = this._dict[attachment.hashCode].effectType;
            }
            this._tips =descStr;
            if(!LayerManager.TopLayer.contains(this._tips)){
    ...
    export function removeTips(e):void{
        if(this._tips != null){
            var onComplete:Function = function(){
                if(LayerManager.TopLayer.contains(this._tips)){
                    LayerManager.TopLayer.removeChild( this._tips );
                    this._tips = null;
                }
            };
